defmodule Chinook.QueryHelpers do
  ...
  defp paginate_order_limit(
         queryable,
         schema,
         key_binding,
         sort_binding,
         args = %{by: cursor_field}
       ) do
    ...
    case args do
      %{last: n} ->
        queryable
        |> order_by([{^key_binding, x}, {^sort_binding, y}],
          desc: field(y, ^cursor_field),
          desc: field(x, ^pk)
        )
        |> limit(^n)
  ...
  defp add_upper_bound(queryable, key_binding, sort_binding, upper_id, key_field, sort_field) do
    ...
    |> join(:inner, [], "upper_bound", as: :upper_bound, on: true)
    |> where(
  ...
  defp add_lower_bound(queryable, key_binding, sort_binding, lower_id, key_field, sort_field) do
    ...
    |> join(:inner, [], "lower_bound", as: :lower_bound, on: true)
    |> where(
